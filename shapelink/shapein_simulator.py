...
# -*- coding: utf-8 -*-
"""
This program shall simulate a ShapeIn instance and
send some test data in the format specified for the
python endpoint.
...
class Shapein_simulator:
    ...
    def register_parameters(self,
        ...
        :return:
        """
        assert len(settings_values) == len(settings_names), "Mismatch setting names and values"
        ...
        msg_stream.writeInt64(MSG_ID_REGISTER)
    ...
    def send_end_of_transmission(self):
        ...
        msg = QtCore.QByteArray()
        msg_stream = QtCore.QDataStream(msg, QtCore.QIODevice.WriteOnly)
        msg_stream.writeInt64(MSG_ID_EOT)
...
ds = dclab.new_dataset("../data/calibration_beads.rtdc")
...
for event_index in range(min(100,len(ds))):
    ...
    vectors = list()
    images = list()
    #print("Prepare data for event: ", event_index)
    ...
    for e in img_features:
        ...
        images.append(i)
    ret = s.send_event(event_index, np.array(scalars), vectors, images)
    #print("Returned: ", ret)
    c += 1
